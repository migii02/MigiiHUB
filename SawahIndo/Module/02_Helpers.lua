...
local Themes = {
    ...
    Monochrome = {
        ...
        -- Warna Tombol & Toggle diganti jadi Graphite / Slate (Abu-abu Kebiruan Elegan)
        ...
        -- Warna gradasi tombol disesuaikan
...
-- ======================================================== --
-- SMART PATCH FIX: MENGGABUNGKAN JUDUL & FIX CALLBACK TOGGLE
-- ======================================================== --
...
-- FIX: Memastikan SetValue pada Toggle juga memicu fungsinya (Callback)
...
UILib._SectionMethods.AddToggle = function(self, config)
    ...
    toggleObj.SetValue = function(obj, v)
        oldSetValue(obj, v) -- Nyalakan animasi toggle
        if config.Callback then config.Callback(v) end -- Jalankan fungsinya!
    end
    return toggleObj
end
    ...
    if not UILib._TabMethods[methodName] then
        UILib._TabMethods[methodName] = function(self, ...)
            if not self._currentSection then
                self._currentSection = self:AddSection({Title = "🛠️ " .. self.Name})
            end
...
-- ========================================= --
--       INISIALISASI WINDOW & TOGGLE        --
-- ========================================= --

local Camera = workspace.CurrentCamera
local ViewportSize = Camera.ViewportSize

-- Hitung persentase layarnya secara manual
local uiWidth = math.floor(ViewportSize.X * 0.56) -- lebar
local uiHeight = math.floor(ViewportSize.Y * 0.78) -- tinggi

-- MEMBACA TEMA YANG DISIMPAN
local savedTheme = "Dark"
local themeFile = "MigiiHub/_ThemeConfig.txt"
if isfolder and not isfolder("MigiiHub") then makefolder("MigiiHub") end
if isfile and isfile(themeFile) then
    pcall(function() savedTheme = readfile(themeFile) end)
end
-- Pastikan tema valid (jika file rusak, kembali ke Dark)
if not Themes[savedTheme] then savedTheme = "Dark" end

Window = UILib.new({
    Title = "MIGII-HUB",
    Subtitle = gameDisplayName,
    Size = UDim2.fromOffset(uiWidth, uiHeight), 
    Theme = savedTheme, -- <-- SEKARANG MENGGUNAKAN TEMA YANG DISIMPAN
    LoadingScreen = false -- Dimatikan karna sekarang menggunakan loader
})

-- ========================================= --
--    TOMBOL LOGO MENGAMBANG (SHOW/HIDE)     --
-- ========================================= --
local ScreenGuiLogo = Instance.new("ScreenGui")
ScreenGuiLogo.Name = "MigiiHubToggleUI"
local successMon2, _ = pcall(function() ScreenGuiLogo.Parent = CoreGui end)
if not successMon2 then ScreenGuiLogo.Parent = player:WaitForChild("PlayerGui") end
...
LogoBtn.Parent = ScreenGuiLogo
Instance.new("UICorner", LogoBtn).CornerRadius = UDim.new(0, 8) 
local LogoStroke = Instance.new("UIStroke")
LogoStroke.Color = Color3.fromRGB(50, 255, 50)
...
LogoBtn.MouseButton1Click:Connect(function()
    ...
    pcall(function()
        if Window._closed or not Window.ScreenGui.Enabled then 
            Window.ScreenGui.Enabled = true 
            Window:_animateOpen() 
        else 
            Window:_animateClose() 
        end
    end)
...
-- ================= TABS =================
local Tabs = {
    Changelog  = Window:AddTab({ Name = "Changelog", Icon = "📝" }), 
    Kaitun     = Window:AddTab({ Name = "Kaitun", Icon = "🔥" }),
    StandAlone = Window:AddTab({ Name = "Stand Alone", Icon = "📦" }),
    EtcFitur   = Window:AddTab({ Name = "ETC Fitur", Icon = "⚙️" }), 
    Teleport   = Window:AddTab({ Name = "Teleport", Icon = "🚀" }), 
    Monitor    = Window:AddTab({ Name = "Monitor", Icon = "📊" }),
    Shop       = Window:AddTab({ Name = "Shop", Icon = "🛒" }),
    Location   = Window:AddTab({ Name = "Location", Icon = "📍" }),
    Config     = Window:AddTab({ Name = "Config", Icon = "⚙" }),
    Webhook    = Window:AddTab({ Name = "Webhook", Icon = "🌐" }),
    ESP        = Window:AddTab({ Name = "ESP", Icon = "👁" })
}
